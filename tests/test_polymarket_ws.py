...
class TestMessageChannelExtraction(unittest.TestCase):
    ...
    def test_current_event_type_field(self):
        self.assertEqual(_message_channel({"event_type": "price_change"}), "price_change")
...
class TestBookEventParsing(unittest.TestCase):
    ...
    def test_current_top_level_book_schema(self):
        client = _make_client()
        changed = client.apply_message({
            "event_type": "book",
            "asset_id": "tok-current",
            "bids": [{"price": "0.61", "size": "8"}],
            "asks": [{"price": "0.63", "size": "9"}],
        })
        self.assertTrue(changed)
        self.assertEqual(client.best_bid_ask("tok-current"), (0.61, 0.63))
...
class TestPriceChangeEventParsing(unittest.TestCase):
    ...
    def test_buy_event_updates_bid(self):
        client = _make_client()
        self._seed_book(client, "tok-1")
        client.apply_message({
            "channel": "price_change",
            "data": {"asset_id": "tok-1", "price": "0.50", "size": "40", "side": "BUY"},
        })
        bid, ask = client.best_bid_ask("tok-1")
        self.assertEqual(bid, 0.50)
        self.assertEqual(ask, 0.51)

    def test_sell_event_updates_ask(self):
        client = _make_client()
        self._seed_book(client, "tok-1")
        client.apply_message({
            "channel": "price_change",
            "data": {"asset_id": "tok-1", "price": "0.51", "size": "50", "side": "SELL"},
        })
        bid, ask = client.best_bid_ask("tok-1")
        self.assertEqual(bid, 0.50)
        self.assertEqual(ask, 0.51)
    ...
    def test_size_zero_removes_level(self):
        ...
        client.apply_message({
            "channel": "price_change",
            "data": {"asset_id": "tok-1", "price": "0.50", "size": "0", "side": "BUY"},
        })
        snap = client.get_book_snapshot("tok-1")
    ...
    def test_current_price_changes_schema(self):
        client = _make_client()
        self._seed_book(client, "tok-1")
        changed = client.apply_message({
            "event_type": "price_change",
            "price_changes": [
                {"asset_id": "tok-1", "price": "0.50", "size": "0", "side": "BUY"},
                {"asset_id": "tok-1", "price": "0.51", "size": "25", "side": "SELL"},
            ],
        })
        self.assertTrue(changed)
        self.assertEqual(client.best_bid_ask("tok-1"), (0.49, 0.51))
...
class TestConnectSubscribeDispatch(unittest.TestCase):
    ...
    def test_subscription_uses_current_market_payload(self):
        client = _make_client()
        ws = _FakeWS()
        client._ws = ws
        asyncio.run(client._send_subscribe(["tok-a", "tok-b"]))
        payload = json.loads(ws.sent[0])
        self.assertEqual(payload["type"], "market")
        self.assertEqual(payload["assets_ids"], ["tok-a", "tok-b"])
